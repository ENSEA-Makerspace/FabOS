...
- The contradiction table was removed, but five operator questions remain explicitly listed in `ROADMAP.md` and `USAGE_RIGHTS_VISION.md`: Admin recovery versus physical safety, Manage→Report, Training catalogue/session localization, policy for existing public identity surfaces, and active-session lifetime after IdP disablement. Do not treat the recommendations as decisions.
