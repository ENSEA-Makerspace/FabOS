...
- Latest delivery is **S106**: `Venue` has a backfilled default row from the physical identity settings; every opening-hour row belongs to it and the existing rendering is unchanged. Next substantive session: **S107**, attach physical resources. All authorization changes stay in shadow mode before activation.
