...
`FeatureWorkspaceRegistry` now drives the operator tabs rendered by `_workspace_tabs.html.twig`; `_admin_list.html.twig` owns the remaining list order, server-side search, progressive advanced filters and bounded active-filter chips. A page supplies data and filters only. The shell never grants access. S128 audited the matrix on Artemis: 30 automated tests / 208 assertions, 188 Twig templates, 29 YAML catalogues/configuration files, and the 14 canonical shared-workspace routes all pass. Phase D (S118–S120) adds feature-scoped Quotas, aggregate Reporting for equipment/spaces, and removes the duplicate global Reservations section while preserving historical redirects.

Phase E (S121–S126) now has one federation boundary. `ProviderRegistry` stores enabled OIDC metadata while secrets are named environment variables; `ExternalIdentityService` links strictly on `(issuer, subject)`, provisions a minimal local account and never imports roles. Public member profiles are disabled by default, expose only five allow-listed fields at `/m/{slug}`, and return `X-Robots-Tag: noindex, nofollow`. `InstanceIdentity` owns the Ed25519 identity; peers, replay nonces, one-use exchanges, audit records and imported projections share the `FABOS_*` tables. Federated credentials are append-only/upserted by source identity and retain expiry/revocation. Machine catalogue imports explicitly discard RFID, token, status, safety, venue, availability and stock fields.

Current exposure inventory: the authenticated profile continues to expose the member's own e-mail, identifier, preferences, reservations, logs and usage rights; admin directories and APIs remain protected by their existing server gates; public catalogues expose only their existing published resource data. The only new anonymous member surface is `/m/{slug}`, requiring both the global opt-in and an allow-list per field. No kiosk endpoint or member-directory API consumes public profiles.
...
**S127 shipped on Artemis (2026-08-10).** This disposable development dataset had one portal, two setting overrides and thirteen feature overrides; they were intentionally purged during consolidation. Portal resolution and scoped storage are removed. Legacy `/admin/portals*` URLs return a permanent transition redirect to `/admin/design/structure`.
...
**S111 persisted shadow packages:** `USAGE_PACKAGE_GRANT` stores action, optional section and venue scope; `USAGE_PACKAGE_GROUP_ASSIGNMENT` gives the same dated/revocable package to a protected/local group. The shadow reader unions direct and group paths only when every scope/date condition is satisfied. It is not connected to the live gate.
