...
**Last updated:** 2026-07-31 (S21–S31, S37; Phases H and U planned) · **Branch:** `fix/creation-upload-duration-and-image` · **Live:** https://fabos.dstei.fr, running `APP_ENV=prod` since 2026-07-30
...
- ⚠️ **The public side has no shared layout at all — 0 of 45 public templates extend `base.html.twig`** (which is 14 lines and does nothing). Every public page is a standalone document re-declaring its own head and stylesheet set. This is the same disease S29 cured in the admin, untouched on the side members actually use. **Phase U S46 owns it; do not "just fix" one page's head in passing.**
- ⚠️ **`symfony/ux-turbo`, `symfony/stimulus-bundle` and `symfony/asset-mapper` are installed and completely unused** — Turbo appears in 0 templates, the only Stimulus controllers are the scaffolding defaults, and `calendrier.html.twig` hand-writes 395 lines of inline JS instead. When interaction work is needed, **reach for the Turbo that is already there**, not for a framework and not for more inline JS. "Twig server-rendered, no SPA" is a deliberate decision — client-side rendering would need the feature gate reimplemented, which is how "visibility is not permission" gets broken by accident.
- **460 KB of CSS across 14 stylesheets, 135 KB of it four overlapping calendar files** (`calendar.css`, `calendar-fix.css`, `calendar-modern.css`, `calendar-leaderboard.css`), plus 5 782 lines of inline `<style>` in 87 templates. A file named `-fix` beside the file it fixes is the shape of the problem.
- ⚠️ **Four of the six homepage blocks are not feature-gated**, and `mini_leaderboard` is a live bug: `leaderboard` is a registry feature, so switching it off hides the nav entry and 404s the page while the homepage keeps rendering a block that links there. Same class as the 404 page's dead escape routes.
