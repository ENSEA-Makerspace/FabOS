...
⬅️ **Surfaces still outside the rule:** `.ev-card` (event detail, inline `<style>`), the badge/formation/machine/reservation/user **div grids** (see below), and the public `.stat-card` on the homepage, which still says `background: white`.
