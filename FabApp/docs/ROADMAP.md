...
**Updated 2026-08-09.** ✅ **S101 makes the S100 access model legible:** User/Global admin remain system accounts; business organisation moves to user groups; fine administrative responsibility sets control management pages and writes; usage packages control resource use, schedules and quotas. The same Volunteers group may assign both, but FabOS keeps two verdicts and two audit sources. The live S97–S99 package system remains unchanged and enforcement remains off.
...
**S67** packages — feature-level integration completed in S97–S98. **S100–S101 fix the next architecture before schema work:** groups as reusable audiences; responsibility sets for fine, scoped administration; separate usage packages; a first-class physical Venue distinct from Portal and reservable Place; canonical equipment categories; complete quota profiles; and shadow/parity migration before activation. A portal administrator is a normal User with portal-scoped responsibilities, never a global admin. See `USAGE_RIGHTS_VISION.md` and the two read-only Development prototypes. Physical-card enforcement remains explicitly deferred. · **S68** lock + no-show (needs RFID reader coverage count; no signal = releases everything) · **S69** archive · **S75** remove favourites (closes the audit's missing-CSRF item — grep routes, not the star).
