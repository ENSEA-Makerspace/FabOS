...
| **Timezone** | box is UTC, app is `Europe/Paris`. Pin the zone on the **read** as well as the write. ⚠️ **Do not "fix" this with a global `date_default_timezone_set()`** — tried and reverted 2026-08-01. `DATETIME` columns carry no zone and Doctrine hydrates them in PHP's default zone, so changing it moves the read *and* the hydration: display is unchanged and every stored date silently changes meaning. ⚠️ **The DB mixes two conventions** — `new \DateTimeImmutable()` (no zone) wrote UTC wall-clock, `new \DateTimeImmutable(…, new \DateTimeZone('Europe/Paris'))` wrote Paris wall-clock. No single default is correct for both. ✅ **The audit is done (S38b in `HISTORY.md`) and needs no data migration** — both conventions are self-consistent. **A = machine timestamps, stored UTC, must be converted on display (this is the bug). B = human-entered wall-clock (`Reservation`, `Event`, `OpeningHours`, access passes), stored in lab time, must stay unpinned (already correct).** ⚠️ Classify **by entity, not by field name**: `dateDebut` belongs to both conventions. Pinning a B field double-shifts it — opening hours would go from `08:00` to `10:00`. |
