...
✅ **Decided 2026-08-05.** **Tiles replace the filters** — two controls for one filter is the question *"which one wins?"* asked of the user; what remains (période, rôle) goes in the search bar. **Dropped columns go to the detail page, later** — ⚠️ between the two steps a datum is visible nowhere (machine token, RFID UID, langue): knowingly, or the lists wait for S58. **Grouped queries are in (S41)** — *"prochaine réservation"* depends on it directly, so **S41 comes before that column, not after**.
