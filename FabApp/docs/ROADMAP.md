...
| ✅ **S134d** | **livré 2026-08-19** : plusieurs plages par jour (l'unicité `(venueId, dayOfWeek)` tombe), `SCHEDULE_EXCEPTION` pour les fermetures datées avec leur raison, et l'écran qui édite les deux. Couverture par UNE plage, jamais l'enveloppe. ⚠️ La **portée attachable** (par workspace, par ressource) n'est PAS faite — `ScheduleResolver` prend un lieu et rien de plus fin ; l'ajouter élargira un paramètre au lieu de réécrire les appelants. | Terra |
