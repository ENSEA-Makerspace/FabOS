...
**Cancelling a *past* booking is a staff act (fixed 2026-08-02 on operator feedback).** The verb service asked `isGranted('ROLE_ADMIN')` and nothing else, so an admin saw *Annuler* on their **own** finished bookings — a records-management control on a personal page. ⚠️ **Role says what you may do; context says what you are currently doing**, and conflating them is what leaked it. New `VerbContext`: `Member` (default everywhere) refuses cancel on any started-or-finished booking whatever your role; `Staff` lifts only the time restriction and is reachable **solely** through `POST /api/staff/reservations/{id}/cancel`, which is `#[IsGranted('ROLE_ADMIN')]` and is what `/admin/reservations` now posts to. ⚠️ It is **not** a security boundary on its own — `IsGranted` is the gate, ownership and state checks still apply. It confines a previously ambient power rather than adding one, and it is **still unaudited**: attribution is S63, staff-on-behalf is S62.

**A page per booking — `GET /reservations/{id}` (added 2026-08-02 on operator feedback).** The card's "Voir" led to the **machine**: a page about a different thing, saying nothing about your slot. The card now links to the booking and carries a single *Gérer* button; the page holds every verb, and the link to the machine/space is demoted to a *fact about the booking*. ⚠️ **Kind-agnostic and verified so** — booking 31 is a *Space* and renders through the same template via `ReservableResolver`; an `{% if %}` on reservable type here is how one page becomes four. ⚠️ **404, not 403,** for a booking that is not yours — a 403 confirms it exists and the id is a small integer. ⚠️ This also **retires one of S51's two queued dialogs instead of porting it**: the ~50-line inline cancel-confirmation `<script>` existed to re-state the booking before you confirmed, and the new page *is* that summary. `confirm_controller.js` does the rest.

**The verb row is `_booking_actions.html.twig`**, shared by the booking page and whatever S62 builds for staff. ⚠️ Ordered by consequence — look · finish · move · cancel — with the irreversible one furthest from the pointer, and *Annuler* the only one in the danger tone. Filled style is reserved for *Terminer maintenant*, the button the lab actually wants pressed.
