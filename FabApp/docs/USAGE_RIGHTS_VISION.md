...
13. **Une Institution peut être descriptive ou connectée à un FabOS.** Une URL unique permet la détection ; les synchronisations connectées sont ensuite confirmées, explicites, signées, idempotentes et limitées aux objets choisis par l'instance et le membre.
...
- **Groupes intégrés :** Manager, Staff, Super user et Formateurs, non supprimables mais localement présentables/configurables ; Formateurs reste rattaché au workspace Formations.
...
Les dimensions d'un grant se combinent avec **AND** : « catégorie Laser au sous-lieu Nord ». Plusieurs grants, packages et sources se combinent avec **OR** et s'accumulent dans le temps : un package autorisant mardi et un autre mercredi autorisent les deux jours. Tout est refusé par défaut hors Admin recovery ; aucun package ne retire un droit et suspension/bannissement restent séparés. Une attribution individuelle ou de groupe peut ajouter une restriction, jamais élargir le grant. Si plusieurs chemins portent des politiques de quotas, chaque chemin grant + calendrier + politique est évalué entièrement ; les champs de politiques ne sont jamais fusionnés. Le chemin gagnant est journalisé selon un ordre stable sans changer le verdict.

Pour Guest, `view`, `register` et les autres actions publiques sont des capacités distinctes. Le réglage FabOS est la valeur héritée, pas un plafond : l'objet peut `inherit`, `allow` ou `deny` chaque action. Avant activation, chaque événement live reçoit `guestView=allow`, puis `guestRegister=allow` si `guestsAllowed=1`, sinon `deny` ; les nouveaux événements héritent ensuite de défauts FabOS fermés. Catalogue/détail GET et inscription POST directe sont testés séparément. Une annulation Guest continue de passer par son lien signé, pas par un compte fictif.
...
Import ponctuel : QR contenant un token aléatoire d'au moins 128 bits dont seul le hash est stocké, un purpose, une audience, une expiration courte et l'identité de l'instance source. Le GET d'aperçu ne consomme jamais le token — les scanners de QR ouvrent parfois les liens. Une confirmation récapitulative unique autorise ensuite l'import automatique des champs généraux proposés (nom, prénom, e-mail, avatar, bio, langue) et des badges acquis. La confirmation POST consomme atomiquement le token ; les conflits sont montrés et aucune valeur locale n'est écrasée silencieusement. Les credentials transportent aussi provenance, `expiresAt`, `revokedAt` et tombstone : un badge expiré/révoqué n'est jamais créé actif, et un nouvel import idempotent applique la révocation source à une copie déjà présente. Le token est révocable, rate-limité, absent des logs/referrers et protégé par `Referrer-Policy: no-referrer`. Une resynchronisation est une nouvelle action manuelle ; la synchronisation continue reste distincte, consentie et révocable.

## Réseau FabOS, badges et machines

L'admin saisit une seule URL sur l'Institution. FabOS la réduit à une origin HTTPS canonique soumise à une contrainte unique, puis tente `/.well-known/fabos`. Un échec conserve l'organisme descriptif, tandis qu'un succès propose de confirmer une connexion interne `FederatedPeer` identifiée séparément par son UUID distant stable. Cette découverte est informative et protégée contre SSRF/DNS rebinding : résolution et IP contrôlées, aucune redirection silencieuse vers une autre origin, timeout et taille limite. Aucune clé ni confiance n'est acceptée sans confirmation explicite. Modifier l'origin suspend le peer existant et force une nouvelle découverte et confirmation.

Une connexion confirmée possède identifiant d'instance, URL d'API allowlistée, clé publique, rotation/révocation des clés, état de confiance, capacités annoncées, règles de partage et journal de synchronisation. L'API est versionnée, signée, protégée contre replay, idempotente et bornée en taille/MIME. Inbox/outbox, quarantaine, conflits, tombstones et retries rendent les pannes partielles explicables.

Deux contrats de partage restent séparés. Pour les claims et credentials personnels, l'export est l'intersection entre l'allowlist du FabOS source et le consentement du membre pour la destination, la catégorie et la durée ; sans l'un des deux, aucune donnée ni identifiant pseudonyme ne sort. Pour les catalogues non personnels — définitions de badges/formations/matériaux et modèles — la politique de publication de l'instance se combine avec le peer/destinataire de confiance, sans consentement membre ni identifiant de personne. Ne jamais synchroniser mots de passe/hashes, secrets MFA/recovery, assertions ou tokens SSO, credentials LDAP, RFID, tokens machines, rôles/groupes/Admin/packages locaux, thèmes/features, arrêts/sécurité machine, stocks locaux, bans/suspensions, réservations détaillées, présences/logs d'accès, notes privées, données financières ou champs non consentis. Aucun rapprochement silencieux par e-mail : une identité externe est `(instanceId, subjectId)` et sa liaison locale demande une confirmation explicite.
